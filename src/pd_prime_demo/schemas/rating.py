...
class RatingFactors(BaseModel):
    ...
    violations: float = Field(default=1.0, ge=0.1, le=5.0, description="Moving violations factor")
    accidents: float = Field(default=1.0, ge=0.1, le=5.0, description="At-fault accidents factor")
    experience: float = Field(default=1.0, ge=0.5, le=2.0, description="Driving experience factor")
    driver_age: float = Field(default=1.0, ge=0.5, le=3.0, description="Driver age factor")

    # Mileage-based factors
    low_mileage: float = Field(default=1.0, ge=0.7, le=1.0, description="Low mileage discount factor")
    high_mileage: float = Field(default=1.0, ge=1.0, le=1.5, description="High mileage surcharge factor")

    # Territory and location factors
    territory: float = Field(default=1.0, ge=0.5, le=2.5, description="Territory risk factor")
    catastrophe_risk: float = Field(default=1.0, ge=1.0, le=1.5, description="Catastrophe risk factor")

    # Vehicle factors
    vehicle_age: float = Field(default=1.0, ge=0.5, le=2.0, description="Vehicle age factor")
    vehicle_type: float = Field(default=1.0, ge=0.8, le=2.0, description="Vehicle type factor")

    # Secondary factors (state-dependent - may be prohibited)
    credit: float | None = Field(default=None, ge=0.6, le=1.4, description="Credit score factor")
    occupation: float | None = Field(default=None, ge=0.9, le=1.1, description="Occupation factor")
    education: float | None = Field(default=None, ge=0.9, le=1.1, description="Education factor")
    marital_status: float | None = Field(default=None, ge=0.9, le=1.1, description="Marital status factor")

    # Prohibited factors (for compliance tracking)
    gender: float | None = Field(default=None, description="Gender factor - prohibited in many states")
    ...
    def calculate_composite_factor(self) -> float:
        ...
        composite = (
            self.violations * self.accidents * self.experience *
            self.driver_age * self.low_mileage * self.high_mileage *
            self.territory * self.vehicle_age * self.vehicle_type
        )
...
class RateTableData(BaseModel):
    ...
    coverages: dict[str, Decimal] = Field(
        ...,
        description="Coverage type to base rate mapping",
        min_length=1
    )
    base_rates: dict[str, Decimal] = Field(
        ...,
        description="Base rates by coverage type",
        min_length=1
    )
    factors: dict[str, "RiskFactorData"] = Field(
        ...,
        description="Rating factors with min/max ranges",
        min_length=1
    )
    ...
    rate_filing_number: str | None = Field(None, max_length=50, description="Regulatory filing number")
    ...
    def validate_rate_structure(self) -> list[str]:
        ...
        for coverage_type, rate in self.coverages.items():
            if rate <= 0:
                errors.append(f"Coverage {coverage_type} rate must be positive, got {rate}")
            if rate > Decimal("10000"):
                errors.append(f"Coverage {coverage_type} rate exceeds maximum (10000), got {rate}")
        ...
        if coverage_types != base_rate_types:
            ...
            if extra_in_base:
                errors.append(f"Extra base rates for unknown coverages: {extra_in_base}")
...
class RiskFactorData(BaseModel):
    ...
    default_value: float = Field(default=1.0, ge=0.1, le=10.0, description="Default factor value")
    description: str | None = Field(None, max_length=255, description="Factor description")
    ...
    def validate_ranges(self) -> list[str]:
        ...
        if self.min_value > self.max_value:
            errors.append(f"Min value {self.min_value} cannot exceed max value {self.max_value}")

        if not (self.min_value <= self.default_value <= self.max_value):
            errors.append(f"Default value {self.default_value} must be between min {self.min_value} and max {self.max_value}")
...
class CoverageRates(BaseModel):
    ...
    # Liability coverages
    bodily_injury: Decimal = Field(..., gt=0, description="Bodily injury liability rate")
    property_damage: Decimal = Field(..., gt=0, description="Property damage liability rate")

    # Optional coverages
    comprehensive: Decimal | None = Field(None, gt=0, description="Comprehensive coverage rate")
    collision: Decimal | None = Field(None, gt=0, description="Collision coverage rate")
    uninsured_motorist: Decimal | None = Field(None, gt=0, description="Uninsured motorist rate")
    personal_injury_protection: Decimal | None = Field(None, gt=0, description="PIP coverage rate")

    # State-specific coverages
    medical_payments: Decimal | None = Field(None, gt=0, description="Medical payments coverage rate")
    property_protection: Decimal | None = Field(None, gt=0, description="Property protection (Michigan)")

    def get_required_coverages(self, state: str) -> dict[str, Decimal]:
        """Get required coverages for a specific state."""
        required_by_state = {
            "CA": {"bodily_injury": self.bodily_injury, "property_damage": self.property_damage,
                   "uninsured_motorist": self.uninsured_motorist or Decimal("0")},
            "TX": {"bodily_injury": self.bodily_injury, "property_damage": self.property_damage},
            "NY": {"bodily_injury": self.bodily_injury, "property_damage": self.property_damage,
                   "personal_injury_protection": self.personal_injury_protection or Decimal("0"),
                   "uninsured_motorist": self.uninsured_motorist or Decimal("0")},
            "FL": {"property_damage": self.property_damage,
                   "personal_injury_protection": self.personal_injury_protection or Decimal("0")},
            "MI": {"bodily_injury": self.bodily_injury, "property_damage": self.property_damage,
                   "personal_injury_protection": self.personal_injury_protection or Decimal("0"),
                   "property_protection": self.property_protection or Decimal("0")},
            "PA": {"bodily_injury": self.bodily_injury, "property_damage": self.property_damage},
        }

        return required_by_state.get(state, {
...
class TerritoryRates(BaseModel):
    ...
    territory_id: str = Field(..., min_length=1, max_length=50, description="Territory identifier")
    ...
    crime_rate: float = Field(default=0.0, ge=-1.0, le=1.0, description="Crime rate risk factor")
    weather_risk: float = Field(default=0.0, ge=-1.0, le=1.0, description="Weather risk factor")
    traffic_density: float = Field(default=0.0, ge=-1.0, le=1.0, description="Traffic density factor")
    catastrophe_risk: float = Field(default=0.0, ge=-1.0, le=1.0, description="Natural disaster risk")

    # ZIP code mappings
    zip_codes: list[str] = Field(..., min_length=1, description="ZIP codes in this territory")
...
class SurchargeFactors(BaseModel):
    ...
    dui_first_offense: Decimal = Field(default=Decimal("0.50"), ge=0, le=2, description="First DUI surcharge rate")
    dui_second_offense: Decimal = Field(default=Decimal("0.75"), ge=0, le=2, description="Second DUI surcharge rate")
    dui_multiple_offense: Decimal = Field(default=Decimal("1.00"), ge=0, le=2, description="Multiple DUI surcharge rate")
    sr22_filing_fee: Decimal = Field(default=Decimal("25.00"), ge=0, description="SR-22 filing fee")

    # High-risk driver surcharges
    high_risk_moderate: Decimal = Field(default=Decimal("0.15"), ge=0, le=1, description="Moderate risk surcharge")
    high_risk_high: Decimal = Field(default=Decimal("0.25"), ge=0, le=1, description="High risk surcharge")
    high_risk_very_high: Decimal = Field(default=Decimal("0.40"), ge=0, le=1, description="Very high risk surcharge")

    # Young driver surcharges by age
    young_driver_under_18: Decimal = Field(default=Decimal("0.50"), ge=0, le=1, description="Under 18 surcharge")
    young_driver_under_21: Decimal = Field(default=Decimal("0.35"), ge=0, le=1, description="Under 21 surcharge")
    young_driver_under_23: Decimal = Field(default=Decimal("0.20"), ge=0, le=1, description="Under 23 surcharge")
    young_driver_under_25: Decimal = Field(default=Decimal("0.10"), ge=0, le=1, description="Under 25 surcharge")

    # Inexperienced driver surcharges
    inexperienced_under_1_year: Decimal = Field(default=Decimal("0.30"), ge=0, le=1, description="Under 1 year licensed")
    inexperienced_under_2_years: Decimal = Field(default=Decimal("0.20"), ge=0, le=1, description="Under 2 years licensed")
    inexperienced_under_3_years: Decimal = Field(default=Decimal("0.10"), ge=0, le=1, description="Under 3 years licensed")

    # Coverage lapse surcharges
    lapse_1_7_days: Decimal = Field(default=Decimal("0.05"), ge=0, le=1, description="1-7 day lapse surcharge")
    lapse_8_30_days: Decimal = Field(default=Decimal("0.10"), ge=0, le=1, description="8-30 day lapse surcharge")
    lapse_31_90_days: Decimal = Field(default=Decimal("0.15"), ge=0, le=1, description="31-90 day lapse surcharge")
    lapse_over_90_days: Decimal = Field(default=Decimal("0.25"), ge=0, le=1, description="Over 90 day lapse surcharge")

    # Vehicle-based surcharges
    vehicle_sports_luxury: Decimal = Field(default=Decimal("0.20"), ge=0, le=1, description="Sports/luxury vehicle surcharge")
    vehicle_commercial_use: Decimal = Field(default=Decimal("0.30"), ge=0, le=1, description="Commercial use surcharge")
    vehicle_modifications: Decimal = Field(default=Decimal("0.15"), ge=0, le=1, description="Modified vehicle surcharge")
...
class SurchargeCalculation(BaseModel):
    ...
    surcharge_type: str = Field(..., min_length=1, max_length=50, description="Type of surcharge")
    driver_id: int | None = Field(None, description="Driver ID (null for policy-level)")
    driver_name: str = Field(..., min_length=1, max_length=100, description="Driver name")
    reason: str = Field(..., min_length=1, max_length=255, description="Reason for surcharge")
    ...
    original_amount: Decimal | None = Field(None, description="Original amount before capping")
...
class PerformanceMetrics(BaseModel):
    ...
    operation_name: str = Field(..., min_length=1, max_length=50, description="Operation being measured")
    ...
    target_ms: float = Field(default=50.0, gt=0, description="Target performance threshold")
    violation_threshold: float = Field(default=0.05, ge=0, le=1, description="Violation rate threshold")
...
class StateRateSettings(BaseModel):
    ...
    state: str = Field(..., min_length=2, max_length=2, description="State code")
    young_driver_age_limit: int = Field(default=25, ge=18, le=30, description="Young driver age limit")

    # Minimum coverage limits
    bodily_injury_per_person: Decimal = Field(..., ge=0, description="BI per person minimum")
    bodily_injury_per_accident: Decimal = Field(..., ge=0, description="BI per accident minimum")
    property_damage: Decimal = Field(..., ge=0, description="Property damage minimum")

    # Optional state requirements
    uninsured_motorist_per_person: Decimal | None = Field(None, description="UM per person minimum")
    uninsured_motorist_per_accident: Decimal | None = Field(None, description="UM per accident minimum")
    personal_injury_protection: Decimal | None = Field(None, description="PIP minimum")
    property_protection: Decimal | None = Field(None, description="Property protection minimum")

    # Prohibited factors
    prohibited_factors: list[str] = Field(default_factory=list, description="Prohibited rating factors")
    ...
    def get_minimum_limits(self) -> dict[str, Decimal]:
        ...
        if self.uninsured_motorist_per_accident is not None:
            limits["uninsured_motorist_per_accident"] = self.uninsured_motorist_per_accident
...
class TerritoryRiskFactors(BaseModel):
    ...
    traffic_density: float = Field(..., ge=-1.0, le=1.0, description="Traffic density factor")
    catastrophe_risk: float = Field(..., ge=-1.0, le=1.0, description="Catastrophe risk factor")
...
class PerformanceThresholds(BaseModel):
    ...
    # Primary thresholds
    target_response_ms: float = Field(default=50.0, gt=0, description="Target response time")
    max_response_ms: float = Field(default=100.0, gt=0, description="Maximum acceptable response time")

    # Violation thresholds
    max_violation_rate: float = Field(default=0.05, ge=0, le=1, description="Maximum violation rate")
    monitoring_window_size: int = Field(default=100, ge=10, le=1000, description="Monitoring window size")

    # Cache thresholds
    min_cache_hit_rate: float = Field(default=0.80, ge=0, le=1, description="Minimum cache hit rate")
    max_cache_size: int = Field(default=50000, ge=1000, description="Maximum cache size")

    # Memory thresholds
    max_memory_mb: float = Field(default=100.0, gt=0, description="Maximum memory usage in MB")
    memory_growth_limit_mb: float = Field(default=1.0, gt=0, description="Memory growth limit per operation")
