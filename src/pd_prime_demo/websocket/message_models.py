...
class QuoteMessage(BaseModel, MessageValidationMixin):
    ...
    action: str = Field(..., pattern="^(subscribe|unsubscribe|update|edit|status_change)$")
    data: Dict[str, Any] = Field(default_factory=dict)
    
    @validator("data")
    @classmethod
    def validate_data_by_action(cls, v: Dict[str, Any], values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if action == "edit":
            ...
            for field in required_fields:
                if field not in v:
                    raise ValueError(f"Field '{field}' is required for status_change action")
...
class RoomMessage(BaseModel, MessageValidationMixin):
    ...
    def validate_room_id_format(cls, v: str) -> str:
        ...
        if room_type not in allowed_types:
            raise ValueError(f"Room type '{room_type}' not allowed. Must be one of {allowed_types}")
...
class AnalyticsMessage(BaseModel, MessageValidationMixin):
    ...
    dashboard_type: str = Field(..., pattern="^(conversion|performance|user_activity|system_health)$")
    action: str = Field(..., pattern="^(start|stop|update|filter)$")
    config: Dict[str, Any] = Field(default_factory=dict)
    
    @validator("config")
    @classmethod
    def validate_config_by_dashboard_type(cls, v: Dict[str, Any], values: Dict[str, Any]) -> Dict[str, Any]:
...
class AdminMessage(BaseModel, MessageValidationMixin):
    ...
    action: str = Field(..., pattern="^(start_monitoring|stop_monitoring|user_activity|performance|system_health)$")
    config: Dict[str, Any] = Field(default_factory=dict)
...
class BinaryMessage(BaseModel, MessageValidationMixin):
    ...
    content_type: str = Field(..., pattern="^[a-zA-Z0-9][a-zA-Z0-9!#$&\\-\\^_]*\/[a-zA-Z0-9][a-zA-Z0-9!#$&\\-\\^_]*$")
