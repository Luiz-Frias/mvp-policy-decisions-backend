...
class ReconnectionConfig(BaseModel):
    ...
    strategy: ReconnectionStrategy = Field(default=ReconnectionStrategy.EXPONENTIAL_BACKOFF)
...
class ConnectionMetrics(BaseModel):
    ...
    last_connection_timestamp: Optional[datetime] = Field(default=None)
    last_disconnection_timestamp: Optional[datetime] = Field(default=None)
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            self.success_rate >= 0.9 and  # 90% success rate
            self.current_uptime_seconds >= 60.0  # At least 1 minute uptime
        )
...
class ReconnectionManager:
    ...
    def __init__(self, config: ReconnectionConfig = None):
        ...
        self.on_reconnection_attempt: Optional[Callable[[int, float], None]] = None
        self.on_reconnection_failed: Optional[Callable[[str], None]] = None
    ...
    @beartype
    async def handle_connection_established(self, connection_id: str, user_id: UUID = None) -> None:
    ...
    async def handle_reconnection_failed(self, error: str) -> None:
        ...
        if (
            self.config.circuit_breaker_enabled and
            self.circuit_breaker_failures >= self.config.circuit_breaker_failure_threshold
        ):
    ...
    def should_attempt_reconnection(self) -> bool:
        ...
        if self.circuit_breaker_state == "open":
            ...
            if (
                ...
                (datetime.now() - self.circuit_breaker_last_failure).total_seconds() >
    ...
    def get_next_delay(self) -> float:
        ...
        if self.config.strategy == ReconnectionStrategy.IMMEDIATE:
            ...
            delay = min(
                self.config.initial_delay_seconds * (self.config.backoff_multiplier ** self.current_attempt),
                self.config.max_delay_seconds
            )
        elif self.config.strategy == ReconnectionStrategy.LINEAR_BACKOFF:
            delay = min(
                self.config.initial_delay_seconds + (self.current_attempt * self.config.initial_delay_seconds),
                self.config.max_delay_seconds
            )
        else:
            delay = self.config.initial_delay_seconds
    ...
    async def get_connection_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_ping_response_time": self._ping_response_times[-1] if self._ping_response_times else None,
            "avg_ping_response_time": sum(self._ping_response_times) / len(self._ping_response_times) if self._ping_response_times else None,
        }
    
    async def _reconnection_loop(self) -> None:
        """Main reconnection loop."""
        while self.state == ConnectionState.DISCONNECTED and self.should_attempt_reconnection():
            try:
                ...
                logger.info(f"Reconnection attempt {self.current_attempt}/{self.config.max_attempts} in {delay:.2f}s")
        ...
        if self.current_attempt >= self.config.max_attempts:
            ...
            logger.error(f"Reconnection failed after {self.config.max_attempts} attempts")
        ...
        while True:
            try:
                if self.state == ConnectionState.CONNECTED:
                    ...
                    response_time = (datetime.now() - ping_start).total_seconds() * 1000  # ms
                    ...
                    await asyncio.sleep(1)  # Check state every second when not connected
    ...
    def get_metrics(self) -> ConnectionMetrics:
        ...
        if self.state == ConnectionState.CONNECTED and self.connection_start_time:
            uptime = (datetime.now() - self.connection_start_time).total_seconds()
            self.metrics = self.metrics.model_copy(update={"current_uptime_seconds": uptime})
