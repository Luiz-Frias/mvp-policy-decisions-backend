...
from .manager import MessagePriority, MessageType, WebSocketMessage
...
class QueueStats(BaseModel):
    ...
    last_processed: Optional[datetime] = Field(default=None)
...
class RedisMessageQueue:
    ...
    def __init__(self, cache: Cache, config: MessageQueueConfig = None):
    ...
    @beartype
    async def enqueue(self, message: WebSocketMessage, connection_id: str) -> Result[UUID, str]:
        ...
        try:
            ...
            queue_name = self._queue_names.get(message.priority, self._queue_names[MessagePriority.NORMAL])
    ...
    @beartype
    async def dequeue(self, timeout_seconds: int = 1) -> Result[Optional[QueuedMessage], str]:
        ...
        try:
            ...
            for priority in [MessagePriority.CRITICAL, MessagePriority.HIGH, MessagePriority.NORMAL, MessagePriority.LOW]:
                ...
                if result:
                    ...
                    await self._cache.lpush(self._processing_queue, queued_msg.model_dump_json())
    ...
    async def acknowledge(self, message_id: UUID) -> Result[None, str]:
        ...
        try:
            ...
            processing_messages = await self._cache.lrange(self._processing_queue, 0, -1)
    ...
    @beartype
    async def reject(self, message_id: UUID, error: str, retry: bool = True) -> Result[None, str]:
            ...
            for msg_data in processing_messages:
                ...
                if queued_msg.id == message_id:
                    ...
                    if retry and queued_msg.should_retry(self._config.max_retries):
                        ...
                        delay = queued_msg.get_retry_delay(self._config.retry_delay_seconds)
                        ...
                        await self._cache.lpush(queue_name, queued_msg.model_dump_json())
                        
                        logger.debug(f"Retrying message {message_id} (attempt {queued_msg.retry_count})")
                    else:
                        # Send to dead letter queue
                        if self._config.dead_letter_queue_enabled:
                            await self._cache.lpush(self._dead_letter_queue, queued_msg.model_dump_json())
                            logger.warning(f"Message {message_id} sent to dead letter queue: {error}")
    ...
    async def get_stats(self) -> Result[List[QueueStats], str]:
        ...
        try:
            ...
            for priority, queue_name in self._queue_names.items():
                ...
                metrics_data = await self._cache.hgetall(f"{self._metrics_key}:{queue_name}")
                
                stats.append(QueueStats(
                    queue_name=queue_name,
                    total_messages=int(metrics_data.get("total_messages", 0)),
                    pending_messages=pending_count,
                    processing_messages=int(metrics_data.get("processing_messages", 0)),
                    failed_messages=int(metrics_data.get("failed_messages", 0)),
                    avg_processing_time_ms=avg_time,
                    last_processed=datetime.fromisoformat(metrics_data["last_processed"]) if metrics_data.get("last_processed") else None
                ))
            ...
            try:
                ...
                for msg_data in processing_messages:
                    try:
                        ...
                        if (
                            queued_msg.processing_started_at and
                            (datetime.now() - queued_msg.processing_started_at).total_seconds() > self._config.processing_timeout_seconds
                        ):
                            # Remove from processing queue and reject
                            await self._cache.lrem(self._processing_queue, 1, msg_data)
                            await self.reject(queued_msg.id, "Processing timeout", retry=True)
                            
                        # Check if message is expired
                        elif queued_msg.is_expired(self._config.message_ttl_seconds):
                            await self._cache.lrem(self._processing_queue, 1, msg_data)
                            logger.warning(f"Expired message {queued_msg.id} removed from processing queue")
    ...
    async def _update_enqueue_metrics(self, queue_name: str) -> None:
        ...
        await self._cache.hincrby(f"{self._metrics_key}:{queue_name}", "total_messages", 1)
    ...
    async def _update_ack_metrics(self, queued_msg: QueuedMessage) -> None:
        ...
        if queued_msg.processing_started_at:
            processing_time = (datetime.now() - queued_msg.processing_started_at).total_seconds() * 1000
    ...
    async def _update_reject_metrics(self, queued_msg: QueuedMessage) -> None:
        ...
        await self._cache.hincrby(f"{self._metrics_key}:{queue_name}", "failed_messages", 1)