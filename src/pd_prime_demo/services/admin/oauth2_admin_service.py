...
class OAuth2AdminService:
    ...
    async def create_oauth2_client(
        ...
        try:
            ...
            await self._db.execute(
                ...
                admin_user_id, datetime.utcnow(), True
            )
    ...
    async def revoke_client_access(
        ...
        try:
            # Verify client exists
            client = await self._db.fetchrow(
                "SELECT id FROM oauth2_clients WHERE client_id = $1",
                client_id
            )
            if not client:
                return Err("Client not found")
            ...
            await self._db.execute(
                ...
                WHERE client_id = $1 AND revoked_at IS NULL
                """,
                client_id, datetime.utcnow(), admin_user_id
            )
            
            # Clear token cache
            await self._cache.delete_pattern(f"oauth2_token:*")
            await self._cache.delete_pattern(f"revoked_token:*")
            
            # Log revocation
            await self._log_oauth2_activity(
                admin_user_id, "revoke_client_tokens", client['id'],
    ...
    async def _log_oauth2_activity(
        ...
        try:
            await self._db.execute(
                ...
                """,
                admin_user_id, action, "oauth2_client", target_id,
