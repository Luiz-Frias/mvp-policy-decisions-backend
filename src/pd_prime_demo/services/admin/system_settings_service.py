...
class SystemSettingsService:
    ...
    def __init__(self, db: Database, cache: Cache, encryption_key: str | None = None) -> None:
    ...
    async def update_setting(
        ...
        try:
            ...
            await self._log_setting_change(
                setting_id, category, key, old_value, serialized,
                updated_by, row["requires_restart"]
            )
    ...
    async def get_category_settings(
        ...
        for row in rows:
            try:
                ...
                if row["is_sensitive"] and self._fernet and value_type == SettingType.ENCRYPTED:
                    decrypted = self._fernet.decrypt(row["value"].encode()).decode()
                    settings[row["key"]] = self._parse_value(decrypted, SettingType.STRING)
                else:
    ...
    async def get_public_settings(self) -> Result[Dict[str, Dict[str, Any]], str]:
        ...
        for row in rows:
            ...
            try:
                value_type = SettingType(row["value_type"])
                settings[category][row["key"]] = self._parse_value(row["value"], value_type)
            except Exception:
                continue
    ...
    def _validate_value(
        ...
        if value_type == SettingType.INTEGER:
            ...
            if "allowed_values" in rules and value not in rules["allowed_values"]:
                return Err(f"Value must be one of: {', '.join(rules['allowed_values'])}")
    ...
    async def _log_setting_change(
        ...
        try:
            await self._db.execute(
                ...
                """,
                setting_id, category, key, old_value, new_value,
                updated_by, requires_restart, datetime.utcnow()
            )
        except Exception:
