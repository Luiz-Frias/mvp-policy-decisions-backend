...
class PremiumCalculator:
    ...
    def apply_multiplicative_factors(
        ...
        all_factor_names = factor_order + [f for f in factors.keys() if f not in factor_order]
...
class CreditBasedInsuranceScorer:
    ...
    def calculate_insurance_score(
        ...
        if credit_utilization <= 0.10:
            ...
            util_adjustment = -50 * (credit_utilization - 0.30)  # High utilization penalty
...
class ExternalDataIntegrator:
    ...
    async def get_crime_risk_factor(
        ...
        try:
            ...
            high_crime_zips = {
                100, 101, 102,  # Boston area
                112, 113, 114,  # Boston suburbs
                200, 201, 202,  # DC area
                100, 104, 105,  # NYC area
                600, 601, 602, 606, 607, 608,  # Chicago area
                900, 901, 902,  # LA area
                941, 945, 946,  # SF Bay area
            }
...
class AIRiskScorer:
    ...
    def _predict_claim_probability(self, features: np.ndarray) -> float:
        ...
        if len(features) > len(coefficients):
            coefficients = np.pad(
                coefficients, (0, len(features) - len(coefficients))
            )
...
class StatisticalRatingModels:
    ...
    def calculate_loss_cost_relativity(
        ...
        try:
            ...
            observed_loss_cost = claim_amount / exposure_years if exposure_years > 0 else 0.0
            ...
            credibility = min(1.0, math.sqrt(claim_count / 16.0))  # 16 claims for full credibility
            ...
            if credibility < credibility_threshold:
                ...
                relativity = credibility * (observed_loss_cost / manual_loss_cost) + (1 - credibility) * 1.0
    ...
    def calculate_frequency_severity_model(
        ...
        try:
            ...
            freq_features = {
                ...
                "urban_indicator": float(1 if territory_profile.get("urban", False) else 0),
            ...
            freq_result = StatisticalRatingModels.calculate_generalized_linear_model_factor(
            ...
            sev_features = {
                ...
                "driver_age": float(driver_profile.get("age", 30)),
                "vehicle_safety_score": float(len(vehicle_profile.get("safety_features", []))),
            }
            ...
            sev_result = StatisticalRatingModels.calculate_generalized_linear_model_factor(
    ...
    def calculate_catastrophe_loading(
        ...
        dwelling_characteristics: Optional[Dict[str, Any]] = None,
    ) -> Result[float, str]:
        ...
        try:
            ...
            if dwelling_characteristics:
                construction_type = dwelling_characteristics.get("construction_type", "wood_frame")
...
class AdvancedPerformanceCalculator:
    ...
    @beartype
    def precompute_lookup_tables(self, table_definitions: Dict[str, Dict[str, Any]]) -> None:
    ...
    def batch_calculate_factors(
        ...
        try:
            ...
            years_licensed = np.array([req.get("years_licensed", 10) for req in factor_requests])
            ...
            for i, request in enumerate(factor_requests):
                result = {
                    ...
                    "combined_factor": float(age_factors[i] * exp_factors[i] * viol_factors[i]),
                }
    ...
    def lookup_factor(self, table_name: str, key: Any) -> Result[float, str]:
        ...
        if isinstance(key, (int, float)):
            sorted_keys = sorted([k for k in table.keys() if isinstance(k, (int, float))])
            if not sorted_keys:
    ...
    def validate_rate_deviation(
        ...
        try:
            ...
            tolerances = {
                ...
                "FL": {"auto": 15.0, "home": 20.0},  # Florida more flexible due to catastrophes
            }
