...
class RatingPerformanceOptimizer:
    ...
    async def optimize_calculation_pipeline(
        ...
        try:
            ...
            driver_task = self._async_driver_scoring(
                quote_data.get("drivers", [])
            )
            vehicle_task = self._async_vehicle_scoring(
                quote_data.get("vehicles", [])
            )
    ...
    async def _async_vehicle_scoring(self, vehicles: List[Dict[str, Any]]) -> List[float]:
    ...
    def get_cache_statistics(self) -> Dict[str, Any]:
        ...
        cache_stats = {
            ...
            "memory_usage_estimate": {
                "calculation_cache_mb": len(self._calculation_cache) * 0.001,  # Rough estimate
                "precomputed_factors_mb": len(self._precomputed_factors) * 0.0005,
            },
        }
        
        # Calculate hit rates if we have performance data
        if "cached_pipeline" in self._performance_metrics and "full_pipeline" in self._performance_metrics:
            ...
            cache_stats["hit_rate"] = cached_calls / total_calls if total_calls > 0 else 0.0
    ...
    async def benchmark_calculation_performance(
        ...
        results = {
            ...
            "throughput_per_second": iterations / (total_time / 1000) if total_time > 0 else 0,
            "performance_target_met": sum(t > 50 for t in timings) / len(timings) < 0.05 if timings else False,  # <5% over 50ms
        }
