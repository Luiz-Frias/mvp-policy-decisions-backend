...
class RatingPerformanceOptimizer:
    ...
    async def get_optimized_base_rate(
        ...
        if not row:
            return Err(
                f"No base rate found for {state} {product_type} {coverage_type}"
            )
    ...
    async def _batch_process_factor_type(
        ...
        if factor_type == "territory":
            ...
            territory_map = {}
            for row in rows:
                territory_map[(row['state'], row['zip_code'])] = float(row['base_factor'])
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        total_cache_ops = self._cache_hit_rates["hits"] + self._cache_hit_rates["misses"]
        cache_hit_rate = (
    ...
    def is_performance_target_met(self, target_ms: int = 50) -> bool:
        ...
        recent_times = self._calculation_times[-100:] if len(self._calculation_times) >= 100 else self._calculation_times
        p95_time = sorted(recent_times)[int(len(recent_times) * 0.95)] if recent_times else 0
