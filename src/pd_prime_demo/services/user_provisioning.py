...
class UserProvisioningService:
    ...
    async def evaluate_provisioning(
        ...
        try:
            ...
            for rule in sorted(rules, key=lambda r: r.priority, reverse=True):
                ...
                if await self._evaluate_conditions(rule.conditions, sso_info, provider_name):
    ...
    async def create_provisioning_rule(
        ...
        try:
            ...
            await self._db.execute(
                ...
                """,
                rule_id, provider_id, rule_name, conditions, actions,
                priority, is_enabled, created_by
            )
    ...
    async def update_provisioning_rule(
        ...
        try:
            ...
            existing = await self._db.fetchrow(
                "SELECT * FROM user_provisioning_rules WHERE id = $1",
                rule_id
            )
            ...
            if "conditions" in updates or "actions" in updates:
                ...
                validation_result = await self._validate_rule(new_conditions, new_actions)
                if isinstance(validation_result, Err):
                    return validation_result
            ...
            for field in ["rule_name", "conditions", "actions", "priority", "is_enabled"]:
                if field in updates:
            ...
            # Clear rules cache
            await self._cache.delete(f"{self._rules_cache_prefix}{existing['provider_id']}")
    ...
    async def delete_provisioning_rule(
        ...
        try:
            ...
            rule = await self._db.fetchrow(
                "SELECT provider_id FROM user_provisioning_rules WHERE id = $1",
                rule_id
            )
            ...
            deleted_count = await self._db.fetchval(
                "DELETE FROM user_provisioning_rules WHERE id = $1 RETURNING 1",
                rule_id
            )
    ...
    async def _get_provisioning_rules(
        ...
        try:
            ...
            await self._cache.set(
                cache_key,
                [rule.model_dump() for rule in rule_objects],
                ttl=300
            )
    ...
    async def _evaluate_conditions(
        ...
        try:
            ...
            if "email_domains" in conditions:
                ...
                user_domain = sso_info.email.split("@")[-1] if "@" in sso_info.email else ""
    ...
    async def _validate_rule(
        ...
        valid_condition_keys = {
            "email_domains", "email_patterns", "required_groups",
            "excluded_groups", "providers", "email_verified", "custom_fields"
        }
        ...
        valid_action_keys = {
            "role", "groups", "auto_create", "warnings", "terminal"
        }
        ...
        if "role" in actions:
            ...
            if actions["role"] not in valid_roles:
                return Err(f"Invalid role: {actions['role']}. Valid roles: {valid_roles}")
