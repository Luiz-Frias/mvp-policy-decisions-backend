...
async def handle_sso_callback(
    ...
    try:
        ...
        if isinstance(tokens_result, Err):
            raise HTTPException(
                status_code=400,
                detail=f"Token exchange failed: {tokens_result.error}"
            )
        ...
        user_result = await sso_manager.create_or_update_user(
            sso_user_info, provider
        )
        if isinstance(user_result, Err):
            raise HTTPException(
                status_code=400,
                detail=f"User provisioning failed: {user_result.error}"
            )
        ...
        jwt_payload = {
            ...
            "auth_method": "sso",
            "provider": provider,
            "scopes": ["read", "write"] + (["admin"] if user.role in ["admin", "system"] else [])
        }
        ...
        await sso_manager._log_auth_event(
            None, "sso", provider, "failed", str(e)
        )
        raise HTTPException(
            status_code=500,
