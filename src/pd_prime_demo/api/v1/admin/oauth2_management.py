...
from ...dependencies import get_current_admin_user, get_oauth2_admin_service, get_db_connection, get_redis
...
class OAuth2ClientCreateRequest(BaseModel):
    ...
    refresh_token_lifetime: int = Field(604800, ge=3600, le=2592000)  # 1 hour to 30 days
    
    @field_validator('allowed_grant_types')
    @classmethod
    def validate_grant_types(cls, v: List[str]) -> List[str]:
        """Validate grant types."""
        valid_grants = {'authorization_code', 'client_credentials', 'refresh_token', 'password'}
...
class OAuth2ClientUpdateRequest(BaseModel):
    ...
    description: Optional[str] = Field(None, max_length=500)
    allowed_grant_types: Optional[List[str]] = Field(None, min_items=1)
    allowed_scopes: Optional[List[str]] = Field(None, min_items=1)
    redirect_uris: Optional[List[str]] = None
    token_lifetime: Optional[int] = Field(None, ge=300, le=86400)
...
async def list_client_certificates(
    ...
    result = await cert_manager.list_client_certificates(
        client_id, include_revoked
    )
