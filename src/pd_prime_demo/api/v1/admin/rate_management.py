...
async def create_rate_table_version(
    ...
    if "rate:write" not in admin_user.effective_permissions:
        raise HTTPException(
            status_code=403,
            detail="Insufficient permissions. Required: rate:write"
        )
...
async def approve_rate_version(
    ...
    if "rate:approve" not in admin_user.effective_permissions:
        raise HTTPException(
            status_code=403,
            detail="Insufficient permissions. Required: rate:approve"
        )
...
async def compare_rate_versions(
    ...
    if "rate:read" not in admin_user.effective_permissions:
        raise HTTPException(
            status_code=403,
            detail="Insufficient permissions. Required: rate:read"
        )
...
async def create_ab_test(
    ...
    if "rate:ab_test" not in admin_user.effective_permissions:
        raise HTTPException(
            status_code=403,
            detail="Insufficient permissions. Required: rate:ab_test"
        )
...
async def get_rate_analytics(
    ...
    if "rate:analytics" not in admin_user.effective_permissions:
        raise HTTPException(
            status_code=403,
            detail="Insufficient permissions. Required: rate:analytics"
        )
...
async def rate_management_health(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail=f"Rate management system unhealthy: {str(e)}"
        )