...
from pydantic import Field, field_validator, EmailStr, computed_field, model_validator
...
DEFAULT_ROLE_PERMISSIONS = {
    ...
    AdminRole.ADMIN: [
        ...
        Permission.QUOTE_READ, Permission.QUOTE_WRITE, Permission.QUOTE_APPROVE,
    ...
    AdminRole.MANAGER: [
        ...
        Permission.QUOTE_READ, Permission.QUOTE_WRITE,
...
class AdminRoleModel(IdentifiableModel):
    ...
    def validate_permissions(cls, v: List[Permission]) -> List[Permission]:
        ...
        if Permission.USER_DELETE in unique_perms and Permission.USER_WRITE not in unique_perms:
            unique_perms.append(Permission.USER_WRITE)
        if Permission.USER_WRITE in unique_perms and Permission.USER_READ not in unique_perms:
...
class AdminUserBase(BaseModelConfig):
    ...
    is_super_admin: bool = Field(
        default=False,
        description="Super admin override - grants all permissions"
    )
    ...
    work_hours_start: Optional[int] = Field(
        None,
        ge=0,
        le=23,
        description="Work hours start (0-23)"
    )
    work_hours_end: Optional[int] = Field(
        None,
        ge=0,
        le=23,
...
class AdminUser(AdminUserBase, IdentifiableModel):
    ...
    password_expires_at: Optional[datetime] = Field(
        None,
        description="Password expiration timestamp"
    )
    ...
    created_by: Optional[UUID4] = Field(
        None,
        description="Admin who created this user"
    )
    ...
    deactivated_by: Optional[UUID4] = Field(
        None,
        description="Admin who deactivated this user"
    )
    ...
    # Relationships
    role: Optional[AdminRoleModel] = Field(
        None,
        description="Populated role information"
    )
    ...
    def is_active(self) -> bool:
        ...
        return (
            self.deactivated_at is None and
            not self.is_locked and
            (self.password_expires_at is None or datetime.utcnow() < self.password_expires_at)
        )
...
class AdminUserCreate(AdminUserBase):
    ...
    role_name: Optional[str] = Field(
        None,
        description="Role name instead of role_id"
    )
    ...
    def validate_password_strength(cls, v: str) -> str:
        ...
        if any(pattern in v.lower() for pattern in ['password', '12345', 'admin', 'qwerty']):
...
class AdminUserUpdate(BaseModelConfig):
    ...
    role_id: Optional[UUID4] = Field(None, description="Update role assignment")
    is_super_admin: Optional[bool] = Field(None, description="Update super admin status")
    two_factor_enabled: Optional[bool] = Field(None, description="Update 2FA status")
...
class SystemSetting(IdentifiableModel):
    ...
    validation_rules: Optional[Dict[str, Any]] = Field(
        None,
        description="Additional validation rules (min, max, regex, etc.)"
    )
    ...
    is_sensitive: bool = Field(
        default=False,
        description="Contains sensitive data (masked in logs)"
    )
    encrypted: bool = Field(
        default=False,
        description="Value is encrypted at rest"
    )
    requires_restart: bool = Field(
        default=False,
        description="Changing this setting requires service restart"
    )

    # Access control
    min_permission: Optional[Permission] = Field(
        None,
        description="Minimum permission required to modify"
    )
    
    # Audit
    last_modified_by: Optional[UUID4] = Field(
        None,
        description="Admin who last modified this setting"
    )
    ...
    version: int = Field(
        default=1,
        ge=1,
        description="Setting version for change tracking"
    )
    ...
    def validate_setting_consistency(self) -> 'SystemSetting':
        ...
        elif self.data_type == 'decimal' and not isinstance(self.value, (Decimal, str, int, float)):
        ...
        if self.validation_rules:
            if 'min' in self.validation_rules and self.value < self.validation_rules['min']:
                raise ValueError(f"Value must be >= {self.validation_rules['min']}")
            if 'max' in self.validation_rules and self.value > self.validation_rules['max']:
            ...
            if 'regex' in self.validation_rules and isinstance(self.value, str):
                ...
                if not re.match(self.validation_rules['regex'], self.value):
                    raise ValueError(f"Value must match pattern: {self.validation_rules['regex']}")
            if 'enum' in self.validation_rules and self.value not in self.validation_rules['enum']:
                raise ValueError(f"Value must be one of: {self.validation_rules['enum']}")
...
class AdminActivityLog(IdentifiableModel):
    ...
    resource_id: Optional[UUID4] = Field(
        None,
    ...
    @classmethod
    def sanitize_sensitive_data(cls, v: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
...
class AdminDashboard(IdentifiableModel):
    ...
    required_permission: Optional[Permission] = Field(
        None,
        description="Permission required to view this dashboard"
    )
    theme: str = Field(
        default="light",
    ...
    is_system: bool = Field(
        default=False,
        description="System dashboard that cannot be deleted"
    )
    
    # Ownership
    created_by: Optional[UUID4] = Field(
        None,
        description="Admin who created the dashboard"
    )
    ...
    @classmethod
    def validate_widget_positions(cls, v: List[DashboardWidget]) -> List[DashboardWidget]:
