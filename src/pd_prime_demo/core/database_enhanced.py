...
class Database:
    ...
    def _get_pool_config(self, pool_type: str = "main") -> PoolConfig:
        ...
        min_conn = max(self.calculate_min_connections(expected_rps), 15)  # Higher minimum based on audit
        ...
        return PoolConfig(
            ...
            connection_timeout=self._settings.database_pool_timeout * 0.8,  # Slightly reduced for faster failure detection
    ...
    async def _warm_connection_pool(self) -> None:
        ...
        try:
            ...
            print(f"🔥 Pool warmed: {successful_warmups}/{target_warmup} connections in {warmup_duration:.1f}ms")
    ...
    @beartype
    async def acquire(self, *, timeout: Optional[float] = None) -> AsyncIterator[asyncpg.Connection]:
        ...
        try:
            async with self._pool.acquire(timeout=timeout) as conn:
                ...
                if len(self._metrics["pool_wait_times_ms"]) > 1000:
                    self._metrics["pool_wait_times_ms"] = self._metrics["pool_wait_times_ms"][-1000:]
                ...
                if len(self._metrics["query_times_ms"]) > 1000:
                    self._metrics["query_times_ms"] = self._metrics["query_times_ms"][-1000:]
    ...
    @contextlib.asynccontextmanager
    @beartype
    async def acquire_read(self, *, timeout: Optional[float] = None) -> AsyncIterator[asyncpg.Connection]:
    ...
    @contextlib.asynccontextmanager
    @beartype
    async def acquire_admin(self, *, timeout: Optional[float] = None) -> AsyncIterator[asyncpg.Connection]:
    ...
    async def execute_with_retry(
        ...
        *args: Any,
        ...
        return Err(f"Connection failed after {max_attempts} attempts: {str(last_error)}")
    ...
    async def get_pool_stats(self) -> PoolMetrics:
        ...
        if self._pool is None:
            return PoolMetrics(
                size=0, free_size=0, min_size=0, max_size=0,
                connections_active=0, connections_idle=0,
                queries_total=0, queries_slow=0,
                pool_exhausted_count=0, average_query_time_ms=0.0,
            )
        
        # Calculate average query time
        avg_query_time = 0.0
        if self._metrics["query_times_ms"]:
            avg_query_time = sum(self._metrics["query_times_ms"]) / len(self._metrics["query_times_ms"])
    ...
    async def get_detailed_pool_metrics(self) -> dict[str, Any]:
        ...
        if total_acquisitions > 0:
            acquisition_error_rate = self._metrics["connection_errors"] / total_acquisitions
        ...
        return {
            ...
            "performance_metrics": {
                ...
                "slow_query_rate": (
                    ...
                    if basic_stats.queries_total > 0 else 0
                ),
            },
            "connection_metrics": {
            ...
            "health_indicators": {
                "is_healthy": basic_stats.pool_exhausted_count == 0 and acquisition_error_rate < 0.01,
                "warning_signs": self._get_warning_signs(basic_stats, acquisition_error_rate),
            },
        }
    ...
    def _get_warning_signs(self, stats: PoolMetrics, error_rate: float) -> list[str]:
        ...
        # High utilization
        utilization = (stats.size - stats.free_size) / stats.size if stats.size > 0 else 0
        if utilization > 0.9:
        ...
        if stats.average_query_time_ms > 100:
            warnings.append(f"High average query time: {stats.average_query_time_ms:.1f}ms")
