    ...
    async def validate_key_permissions(
        ...
        try:
            ...
            for permission in required_permissions:
                if not ScopeValidator.check_scope_permission(key_info["scopes"], permission):
                    missing_permissions.append(permission)
            
            if missing_permissions:
                return Err(f"Missing required permissions: {', '.join(missing_permissions)}")
    ...
    async def verify_key_ownership(
        ...
        try:
            key_client_id = await self._db.fetchval(
                "SELECT client_id FROM api_keys WHERE id = $1",
                key_id
            )
    ...
    async def create_scoped_api_key(
        ...
        try:
            ...
            if not requested_scopes.issubset(parent_scopes):
                ...
                return Err(f"Scopes not allowed by parent key: {', '.join(invalid_scopes)}")
