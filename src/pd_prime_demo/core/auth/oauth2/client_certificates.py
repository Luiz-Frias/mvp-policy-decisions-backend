...
class ClientCertificateManager:
    ...
    async def register_client_certificate(
        ...
        try:
            ...
            existing_cert = await self._db.fetchrow(
                ...
                """,
                client_id, cert_info["fingerprint"]
            )
            ...
            cert_id = await self._db.fetchval(
                ...
                """,
                client_id, certificate_pem, certificate_name,
                cert_info["fingerprint"], cert_info["subject_dn"],
                cert_info["issuer_dn"], cert_info["valid_from"],
                cert_info["valid_until"], admin_user_id,
    ...
    async def validate_client_certificate(
        ...
        try:
            ...
            # Check cache first
            cached_cert = await self._cache.get(f"{self._cert_cache_prefix}{fingerprint}")
    ...
    async def revoke_client_certificate(
        ...
        try:
            ...
            await self._db.execute(
                ...
                WHERE id = $1
                """,
                certificate_id, datetime.now(timezone.utc), admin_user_id, reason
            )
            
            # Invalidate cache
            await self._cache.delete(f"{self._cert_cache_prefix}{cert_info['fingerprint']}")
    ...
    async def create_certificate_signing_request(
        ...
        try:
            ...
            subject_components.append(x509.NameAttribute(NameOID.COMMON_NAME, client_id))
