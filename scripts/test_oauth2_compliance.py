...
from src.pd_prime_demo.core.auth.oauth2.scopes import ScopeValidator, SCOPES  
...
async def test_oauth2_rfc_compliance():
        ...
        if auth_code_result.is_ok():
            ...
            if token_result.is_ok():
                ...
                if 'refresh_token' in tokens:
                    print(f"   ✓ Refresh token issued: {tokens['refresh_token'][:20]}...")
            else:
                print(f"   ✗ Token exchange failed: {token_result.error}")
        else:
            print(f"   ✗ Authorization code generation failed: {auth_code_result.error}")
    else:
    ...
    if password_client_result.is_ok():
        ...
        print(f"   ✗ Password grant client creation failed: {password_client_result.error}")
    ...
    if cc_client_result.is_ok():
        ...
        if cc_token_result.is_ok():
            ...
            print(f"   ✓ Client credentials token issued: {tokens['access_token'][:20]}...")
        else:
            print(f"   ✗ Client credentials grant failed: {cc_token_result.error}")
    else:
        print(f"   ✗ Client credentials client creation failed: {cc_client_result.error}")
    ...
    if 'tokens' in locals():
        ...
        if introspection_result.is_ok():
            ...
            print(f"   ✓ Token introspection successful: active={token_info.get('active')}")
        else:
    ...
    if pkce_result.is_ok():
        ...
        print(f"   ✓ PKCE code verifier generated: {pkce_params['code_verifier'][:20]}...")
        print(f"   ✓ PKCE code challenge generated: {pkce_params['code_challenge'][:20]}...")
    ...
    if missing_client_result.is_err() and "client_id is required" in missing_client_result.error:
