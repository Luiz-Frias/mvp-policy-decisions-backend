...
class WebSocketLoadTester:
    ...
    @beartype
    async def create_connection(self, session: aiohttp.ClientSession, user_id: str) -> bool:
    ...
    async def run_connection_test(self, target_connections: int) -> dict[str, Any]:
        ...
        connector = aiohttp.TCPConnector(
            limit=target_connections + 100,
            limit_per_host=target_connections + 100
        )
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            ...
            for i in range(0, target_connections, batch_size):
                ...
                print(f"  Batch {i//batch_size + 1}: {successful_in_batch}/{len(batch_tasks)} connections successful")
            ...
            if self.active_connections:
                print(f"📡 Testing message latency with {len(self.active_connections)} connections...")
        ...
        return {
            ...
            "p95_connection_time": statistics.quantiles(self.connection_times, n=20)[18] if len(self.connection_times) > 20 else 0,
            ...
            "p95_message_latency_ms": statistics.quantiles(self.message_latencies, n=20)[18] if len(self.message_latencies) > 20 else 0,
            "memory_usage": {
    ...
    def print_results(self, results: dict[str, Any]) -> None:
        ...
        print(f"📈 Success Rate: {(results['successful_connections'] / results['target_connections']) * 100:.1f}%")
        ...
        print(f"   Connections/sec: {results['successful_connections']/results['connection_time']:.1f}")
        ...
        success_rate = (results['successful_connections'] / results['target_connections']) * 100
        print(f"   Connection Success Rate: {success_rate:.1f}% {'✅' if success_rate >= 95 else '❌'} (target: ≥95%)")
        
        avg_latency = results['avg_message_latency_ms']
        print(f"   Avg Message Latency: {avg_latency:.1f}ms {'✅' if avg_latency <= 50 else '❌'} (target: ≤50ms)")
        
        p95_latency = results['p95_message_latency_ms']
        print(f"   P95 Message Latency: {p95_latency:.1f}ms {'✅' if p95_latency <= 100 else '❌'} (target: ≤100ms)")
        
        memory_per_conn = memory['increase_mb'] / results['successful_connections'] if results['successful_connections'] > 0 else 0
        print(f"   Memory per Connection: {memory_per_conn*1024:.1f}KB {'✅' if memory_per_conn <= 0.1 else '❌'} (target: ≤100KB)")
        
        conn_rate = results['successful_connections'] / results['connection_time']
        print(f"   Connection Rate: {conn_rate:.1f}/sec {'✅' if conn_rate >= 100 else '❌'} (target: ≥100/sec)")


@click.command()
@click.option('--url', default='ws://localhost:8000', help='WebSocket server URL')
@click.option('--connections', default=1000, help='Number of concurrent connections to test')
