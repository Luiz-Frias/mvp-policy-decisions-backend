#!/usr/bin/env python3
"""Script to fix Python 3.12+ type annotations for Python 3.11 compatibility."""
...
REPLACEMENTS = [
    ...
    # Optional patterns (these are already compatible with 3.11, but included for completeness)
    ...
    if used_types and content != original_content:
        ...
        if import_match:
            ...
            if 'from typing import' not in content and 'import typing' not in content:
                ...
                import_section_match = re.search(r'^((?:from .+ import .+|import .+)\n)+', content, re.MULTILINE)
                if import_section_match:
                    ...
                    needed_imports = sorted(used_types)
                    lines.insert(insert_line, f"from typing import {', '.join(needed_imports)}")
                    content = '\n'.join(lines)
