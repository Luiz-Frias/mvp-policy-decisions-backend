...
class ConnectionPoolBenchmark:
    ...
    async def simulate_connection_acquisition(
        ...
        print(f"🔍 Testing pool_size={pool_size}, concurrent_requests={concurrent_requests}")
            ...
            try:
                ...
                async with asyncio.timeout(1.0):  # 1 second timeout
                    async with semaphore:
                        ...
                        await asyncio.sleep(0.01 + (time.perf_counter() % 0.05))  # 10-60ms queries
        ...
        return {
            ...
            "timeout_rate": timeouts / (successful_acquisitions + timeouts) if (successful_acquisitions + timeouts) > 0 else 0,
        }
    ...
    async def run_benchmark_suite(self) -> Dict[str, Any]:
        ...
        for pool_size, concurrent_requests in scenarios:
            ...
            print(f"Pool {pool_size:2d} | Concurrency {concurrent_requests:4d} | "
    ...
    @beartype
    def _find_optimal_configuration(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for result in results:
            ...
            throughput_score = min(result["throughput_rps"] / 1000, 1.0)  # Max score at 1000 RPS
            latency_score = max(0, 1.0 - (result["p95_response_time_ms"] / 100))  # Penalty for > 100ms
            timeout_score = max(0, 1.0 - (result["timeout_rate"] * 10))  # Heavy penalty for timeouts
