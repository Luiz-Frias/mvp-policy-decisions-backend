...
class PerformanceTester:
    ...
    async def test_rate_tables_indexes(self) -> None:
        ...
        await self.run_query_with_timing(
            ...
            """,
            "CA", "auto", "liability", datetime.utcnow().date()
        )
    ...
    async def test_analytics_indexes(self) -> None:
        ...
        await self.run_query_with_timing(
            ...
            """,
            "quote_created", "conversion", datetime.utcnow() - timedelta(days=1)
        )
    ...
    def print_summary(self) -> None:
        ...
        for r in slow_queries:
            print(f"   - {r['test']}: {r['avg_time_ms']:.2f}ms {'(NO INDEX!)' if not r['index_used'] else ''}")
        ...
        print(f"   - Queries using indexes: {len(self.results) - len(no_index)}/{len(self.results)}")
