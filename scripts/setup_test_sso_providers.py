...
async def setup_test_providers() -> None:
    ...
    try:
        ...
        for provider in providers:
            ...
            print(f"✅ Created SSO provider: {provider['provider_name']} ({provider_id})")
            ...
            if provider["provider_name"] == "google":
                await conn.execute(
                    ...
                    """,
                    uuid4(), provider_id, "admins@testcorp.com", "admin", True
                )
                await conn.execute(
                    """
                    INSERT INTO sso_group_mappings (
                    ...
                    """,
                    uuid4(), provider_id, "underwriters@testcorp.com", "underwriter", True
                )
        ...
        print("\nTest URLs:")
        for provider in providers:
            print(f"  {provider['provider_name']}: /auth/sso/{provider['provider_name']}/initiate")
