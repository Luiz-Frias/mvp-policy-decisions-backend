...
def analyze_migration_function(node: ast.FunctionDef, func_type: str) -> Dict[str, List[str]]:
...
def validate_migration_sequence(migrations: List[Dict[str, Any]]) -> List[str]:
    ...
    if len(base_migrations) != 1:
        errors.append(f"Expected exactly 1 base migration, found {len(base_migrations)}")
...
def has_circular_dependency(graph: Dict[str, Optional[str]]) -> bool:
    """Check for circular dependencies in migration graph."""
    visited = set()
    rec_stack = set()
    
    def visit(node: Optional[str]) -> bool:
        if node is None:
            return False
...
def check_required_tables() -> List[str]:
    ...
    required_tables = {
        # Core tables
        "customers", "policies", "claims", "users",
        # Quote system
        "quotes",
        # Rating engine
        "rate_tables", "discount_rules", "surcharge_rules", "territory_factors",
        # Security & compliance
        "sso_providers", "oauth2_clients", "user_mfa_settings", "audit_logs",
        # Real-time & analytics
        "websocket_connections", "analytics_events",
        # Admin system
        "admin_users", "admin_roles", "admin_permissions", "system_settings",
        "admin_activity_logs", "admin_dashboards", "admin_rate_approvals"
    }
...
def validate_migration_files():
    ...
    for file_path in migration_files:
        ...
        print(f"  📋 Revision: {info['revision']}, Down revision: {info['down_revision']}")
    ...
    if errors:
        ...
        print(f"  • {len(set().union(*[m.get('tables_created', []) for m in migrations]))} total tables")
        print(f"  • {sum(len(m.get('indexes_created', [])) for m in migrations)} total indexes")
        print(f"  • {sum(len(m.get('functions_created', [])) for m in migrations)} total functions")
        print(f"  • {sum(len(m.get('triggers_created', [])) for m in migrations)} total triggers")
        return True
