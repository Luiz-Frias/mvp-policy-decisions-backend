...
class TestRatingEnginePerformance:
    ...
    async def test_bulk_quote_performance(self, rating_engine: RatingEngine):
        ...
        tasks = [
            rating_engine.calculate_premium(**scenario)
        ...
        calculation_times = [
            r.ok_value.calculation_time_ms for r in successful_results
        ]
        ...
        assert avg_time < 30, f"Average calculation time {avg_time:.1f}ms exceeds 30ms target"
        assert p95_time < 45, f"P95 calculation time {p95_time:.1f}ms exceeds 45ms target"
        assert p99_time < 50, f"P99 calculation time {p99_time:.1f}ms exceeds 50ms target"
        
        # Verify throughput
        throughput = len(successful_results) / (total_time_ms / 1000)
        assert throughput > 50, f"Throughput {throughput:.1f} quotes/sec below 50/sec target"
