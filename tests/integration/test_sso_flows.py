...
class TestSSOIntegrationFlows:
    ...
    async def test_complete_sso_flow_new_user(
        ...
        mock_db.fetchrow.side_effect = [
            ...
            {"id": uuid4(), "email": "john.doe@example.com", "first_name": "John", 
             "last_name": "Doe", "role": "agent", "is_active": True}  # Created user
        ]
        ...
        # Test user creation
        result = await sso_manager.create_or_update_user(
            sample_sso_user_info, "google"
        )
    ...
    async def test_sso_flow_existing_user_update(
        ...
        mock_db.fetchrow.side_effect = [
            {"id": user_id, "email": "john.doe@example.com", "first_name": "John",
             "last_name": "Doe", "role": "agent", "is_active": True},  # Existing user
            {"id": user_id, "email": "john.doe@example.com", "first_name": "John",
             "last_name": "Doe", "role": "agent", "is_active": True}  # Updated user
        ]
    ...
    async def test_sso_flow_auto_create_disabled(
        ...
        mock_db.fetchrow.side_effect = [
            None,  # No existing SSO link
            None,  # No existing email user
            {"auto_create_users": False, "allowed_domains": [], "default_role": "agent"}  # Auto-create disabled
        ]
    ...
    async def test_sso_flow_domain_restriction(
        ...
        mock_db.fetchrow.side_effect = [
            None,  # No existing SSO link
            None,  # No existing email user
            {"auto_create_users": True, "allowed_domains": ["example.com"], "default_role": "agent"}  # Domain restricted
        ]
        
        # Test user creation failure due to domain
        result = await sso_manager.create_or_update_user(
            restricted_user, "google"
        )
    ...
    async def test_group_mapping_assignment(
        ...
        mock_db.fetchrow.side_effect = [
            None,  # No existing SSO link
            None,  # No existing email user
            {"auto_create_users": True, "allowed_domains": [], "default_role": "agent"},  # Auto-create enabled
            {"id": user_id, "email": "john.doe@example.com", "first_name": "John",
             "last_name": "Doe", "role": "agent", "is_active": True}  # Created user
        ]
        mock_db.fetchval.side_effect = [
    ...
    async def test_provider_configuration_validation(
        self,
        sso_manager,
        mock_db
    ):
    ...
    async def test_sso_state_validation(self, mock_cache):
        ...
        from src.pd_prime_demo.api.v1.sso_auth import handle_sso_callback
